...
class AnimatedStatusFixed {
	...
	async ResolveStatusField(text = "") {
		...
		try {
			return eval(text.substr(Prefix.length));
		} catch (e) {
...
const Status = {
	strerror: (req) => {
		...
		for (const s of ["errors", "custom_status", "text", "_errors", 0, "message"])
			if ((json == undefined) || ((json = json[s]) == undefined))
				return "Unknown error. Please report at |Discord: kufox| or github.com/KuFoX-kfx/Animated-Status-Fixed";
	...
	Set: async (status) => {
		...
		req.onload = () => {
			...
			if (err != undefined)
				BdApi.showToast(`Animated Status Fixed: Error: ${err}`, {type: "error"});
		};
...
const GUI = {
	...
	UsernameInput: (text = "", placeholder = "") => {
		let input = document.createElement("input");
		input.value = String(text);
		input.placeholder = String(placeholder);
		return input;
